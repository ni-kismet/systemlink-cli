...
through the terminal or CLI arguments. 3. Complete the normal Stratus login in the browser and confirm that the CLI
receives a callback, exchanges the code, and successfully probes the API
URL with an `Authorization: Bearer` header once the API gateway is deployed. 4. Run a normal resource command, then repeat it after the access-token
